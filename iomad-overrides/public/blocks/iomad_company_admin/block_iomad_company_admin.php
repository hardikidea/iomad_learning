...
class block_iomad_company_admin extends block_base {
    ...
    /**
     * Build capability-filtered tabs declared by plugin IOMAD menu entries.
     *
     * Core IOMAD tabs remain unchanged. A plugin can opt into a separate tab by
     * declaring tabcustom, tablabel, and tabicon on at least one menu entry.
     *
     * @param array $menus IOMAD menu entries.
     * @param context $companycontext Active company context.
     * @return array
     */
    private function get_custom_tabs(array $menus, context $companycontext): array {
        $tabs = [];
        foreach ($menus as $menu) {
            if (empty($menu['tabcustom']) || empty($menu['tablabel']) || empty($menu['category'])) {
                continue;
            }
            $tabid = (int)($menu['tab'] ?? 0);
            if ($tabid < 1 || isset($tabs[$tabid])) {
                continue;
            }
            if (empty($menu['cap']) || !iomad::has_capability($menu['cap'], $companycontext)) {
                continue;
            }
            $tabs[$tabid] = [
                'category' => clean_param((string)$menu['category'], PARAM_ALPHANUMEXT),
                'icon' => clean_param((string)($menu['tabicon'] ?? 'fa-cog'), PARAM_ALPHANUMEXT),
                'label' => (string)$menu['tablabel'],
            ];
        }
        ksort($tabs, SORT_NUMERIC);
        return $tabs;
    }
    ...
    public function get_content() {
        ...
        // Resolve a submitted company change before capability and custom-menu checks.
        ...
        $menus = $this->get_menu();
        $customtabs = $this->get_custom_tabs($menus, $companycontext);
        ...
        foreach ($customtabs as $tabid => $customtab) {
            $customtab['selected'] = $selected;
            $tabs[] = $customtab;
            $panes[$tabid] = [
                'category' => $customtab['category'],
                'items' => [],
                'selected' => $selected,
            ];
            $selected = false;
        ...
        foreach ($menus as $key => $menu) {
            ...
            if ((!get_config('local_iomad', 'useicons') || !empty($menu['preferfonticon'])) &&
                !empty($menu['icon'])) {
                $icon = $menu['icon'];
            ...
            $menu['icon'] = $icon;
            // Mustache escapes attribute values. Passing a moodle_url object would
            // escape its already-encoded ampersands a second time.
            $menu['url'] = $url->out(false);
        ...
        foreach ($panes as $paneid => $paneentry) {
            if (empty($paneentry['items'])) {
                unset($panes[$paneid]);
                $doreset = true;
                foreach ($tabs as $tabindex => $tabentry) {
                    if ($tabentry['category'] !== $paneentry['category']) {
                        continue;
                    }
                    if ($tabentry['selected']) {
                        $doselected = true;
                    }
                    unset($tabs[$tabindex]);
                    break;
