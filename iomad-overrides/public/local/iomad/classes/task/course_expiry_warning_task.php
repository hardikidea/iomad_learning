        ...
        foreach ($enabledcompanies as $enabledcompany) {
            ...
            foreach ($expirycourses as $expirycourse) {
                ...
                foreach ($allusers as $compuser) {
                    ...
                    // Resolve the records required by the expiry event and email template.
                    $user = $DB->get_record('user', ['id' => $compuser->userid], '*', MUST_EXIST);
                    $course = $DB->get_record('course', ['id' => $compuser->courseid], '*', MUST_EXIST);
