...
class importer {
    ...
    public function apply(bool $dryrun = false): array {
        ...
        try {
            ...
            $this->apply_policies();
            $this->apply_licenses();
            $this->apply_tenantmaster_profiles();
            $transaction->allow_commit();
        } catch (\Throwable $exception) {
            $transaction->rollback($exception);
        }

        // IOMAD defers user_created observers until the transaction commits.
        // Its signup observer normalises new company memberships to user type,
        // so canonical manager types must be reconciled after those observers.
        $this->reconcile_user_memberships();
    ...
    private function apply_companies(): void {
        ...
        foreach ($this->pack->rows('companies') as $row) {
            ...
            if ($record) {
                $data->id = $record->id;
                $comparisondata = clone($data);
                unset($comparisondata->companydomains);
                $desireddomains = $domains[$shortname] ?? [];
                $currentdomains = $DB->get_fieldset_select(
                ...
                sort($desireddomains);
                if (
                    !$this->record_changed($record, $comparisondata)
                    && $currentdomains === $desireddomains
                ) {
                    $this->bump('companies', 'unchanged');
                    continue;
                }
    ...
    /**
     * Register imported native companies with Tenant Master.
     *
     * This is optional for deployments that do not install local_tenantmaster.
     * Existing profiles and adopted defaults are never overwritten.
     */
    private function apply_tenantmaster_profiles(): void {
        global $DB;

        if (
            !class_exists(onboarding_service::class)
                || !$DB->get_manager()->table_exists('local_tenantmaster_tenant')
        ) {
            return;
        }

        $institutiontypes = [];
        foreach ($this->pack->rows('institutions') as $institution) {
            $institutiontypes[$institution['institution_id']] = $institution['type'] ?: 'training';
        }
        $service = new onboarding_service();
        foreach ($this->pack->rows('companies') as $companyrow) {
            $companyid = $this->company_id($companyrow['company_shortname']);
            if ($companyid <= 0) {
                continue;
            }
            $existing = $DB->record_exists('local_tenantmaster_tenant', ['companyid' => $companyid]);
            $tenanttype = $institutiontypes[$companyrow['institution_id']] ?? 'training';
            $service->adopt_existing($companyid, $tenanttype);
            $this->bump('tenantmaster_profiles', $existing ? 'unchanged' : 'create');
        }
    }
    ...
    private function apply_users(): void {
        ...
        foreach ($this->pack->rows('users') as $row) {
            ...
            $desireddepartmentid = $departmentid ?: company::get_company_parentnode($companyid)->id;
            $desiredmanagertype = $this->manager_type($rolekey);
            $desirededucator = in_array($rolekey, ['teacher_faculty'], true);
            ...
            $user = (object)[
                ...
                'managertype' => $desiredmanagertype,
                'educator' => $desirededucator ? 1 : 0,
            ...
            if ($userid) {
                ...
                $userid = company_user::create($user, $companyid);
    ...
    /**
     * Reconcile canonical company memberships after deferred IOMAD observers.
     */
    private function reconcile_user_memberships(): void {
        ...
        foreach ($this->pack->rows('users') as $row) {
            ...
            $departmentid = $this->department_id($companyid, $row['department_shortname'] ?? '');
            if (!$departmentid) {
                $departmentid = (int)company::get_company_parentnode($companyid)->id;
            }
            $service->reconcile(
                $userid,
                $companyid,
                $departmentid,
                $this->manager_type($row['role_key']),
                in_array($row['role_key'], ['teacher_faculty'], true),
            );
            $this->bump('company_memberships', 'reconciled');
        }
    }
    ...
    private function apply_parent_links(): void {
        foreach ($this->pack->rows('parent_links') as $row) {
            ...
            $roleid = $this->role_id('tenantguardian');
    ...
    private function ensure_custom_roles(): void {
        ...
        $parentrole = $this->role_id('tenantguardian');
        if (!$parentrole) {
            $parentrole = create_role(
                'Tenant guardian',
                'tenantguardian',
                'Explicit learner mentor relationship for parent and guardian access.'
            );
        ...
        if (get_capability_info('moodle/user:viewalldetails')) {
            assign_capability(
    ...
    /**
     * Return whether any supplied API field differs from its persisted value.
     *
     * @param object $record Existing record.
     * @param object $data Proposed API data.
     * @return bool
     */
    private function record_changed(object $record, object $data): bool {
    ...
    private function manager_type(string $rolekey): int {
        return match ($rolekey) {
            'principal_registrar' => 1,
            'trustee_management' => 4,
            'hod_dean' => 2,
            default => 0,
        };
    }
