...
final class report_engine_test extends \advanced_testcase {
    ...
    public function test_reports_enforce_company_user_and_course_boundaries(): void {
        ...
        $masked = new tenant_scope($companya->id, get_admin()->id, false, [], false);
        foreach (array_keys(report_catalog::all()) as $reportkey) {
            $result = $engine->generate($reportkey, $masked, $filters);
            $serialized = json_encode($result->get_rows(), JSON_THROW_ON_ERROR);
            $this->assertStringNotContainsString('Scoped Learner', $serialized, $reportkey);
            $this->assertStringNotContainsString('scoped@example.test', $serialized, $reportkey);
        }
        $maskedstudent = $engine->generate('student_engagement', $masked, $filters);
        $this->assertMatchesRegularExpression(
            '/^Learner [A-F0-9]{12}$/',
            $maskedstudent->get_rows()[0]['learner']
        );
        $this->assertSame('', $maskedstudent->get_rows()[0]['email']);
        $this->assertSame(
            $maskedstudent->get_checksum(),
            $engine->generate('student_engagement', $masked, $filters)->get_checksum()
        );
    }
