...
final class report_engine {
    ...
    private function learner_report(tenant_scope $scope, array $filters): report_result {
        $tracks = $this->tracks($scope, $filters);
        $users = $this->users_by_id(array_map(static fn(object $track): int => (int)$track->userid, $tracks));
        $courses = $this->courses(array_map(static fn(object $track): int => (int)$track->courseid, $tracks));
        $rows = [];
        foreach ($tracks as $track) {
            $user = $users[(int)$track->userid] ?? null;
            $rows[] = [
                'learner' => $user ? fullname($user) : get_string('deleteduser', 'local_tenantanalytics'),
                'email' => $user->email ?? '',
    ...
    private function time_course(tenant_scope $scope, array $filters): report_result {
        ...
        $users = $this->users_by_id(array_column($events, 'userid'));
    ...
    private function users_by_id(array $ids): array {
        ...
        $fields = 'id,firstname,lastname,firstnamephonetic,lastnamephonetic,middlename,alternatename,email';
