...
function xmldb_local_tenantanalytics_upgrade(int $oldversion): bool {
    ...
    if ($oldversion < 2026072502) {
        upgrade_plugin_savepoint(true, 2026072502, 'local', 'tenantanalytics');
    }
    if ($oldversion < 2026072503) {
        if (!get_config('local_tenantanalytics', 'pseudonymkey')) {
            set_config('pseudonymkey', bin2hex(random_bytes(32)), 'local_tenantanalytics');
        }
        upgrade_plugin_savepoint(true, 2026072503, 'local', 'tenantanalytics');
    }
    return true;
}
