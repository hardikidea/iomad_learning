...
if docker compose ps --status running --quiet iomad | grep -q .; then
    ...
    active_image_id="$(docker compose images -q iomad | head -1)"
    active_image_commit="$(
        docker image inspect "${active_image_id}" \
...
docker compose images --format json > "${BACKUP_DIR}/active-images.json" 2>/dev/null || true
